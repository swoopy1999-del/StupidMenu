...
let infAmmo = true;
let gunColor: [number, number, number, number] = [0.08, 0.08, 0.08, 0.75];
...
let ejectDupeValues: [number, number, number, number, number, number, number, number] = [1, 2, 5, 10, 25, 64, 100, 128]
...
let headSizeValues: [number, number, number, number, number, number, number, number, number] = [1, 2, 5, 10, 15, 20, 25, 50, 0]
...
let visualizer: any[] = []; // persistent cube
...
//
    ...
    function world2Player(position) {
        ...
        position = Vector3.method("op_Addition", 2).invoke(position, getTransform(GTPlayer
    ...
    const loadedBundles: { [key: string]: any } = {};
    const loadedClips: { [key: string]: any } = {};
...
function renderGun(overrideLayerMask = null) {
    ...
    for (const hit of hits) {
        const distance = Vector3.method("Distance").invoke(
            hit.method("get_point").invoke(),
            StartPosition
        );
        try {
    ...
    if (finalRay != null) {
        ...
        const fallback = Vector3.method("op_Addition").invoke(
            StartPosition,
            Vector3.method("op_Multiply").invoke(Direction, 10.0)
        );
        ...
        return () => {
            try {
                ...
                leftDevice.method("TryGetFeatureValue", 2).invoke(
                    CommonUsages.field("primary2DAxis").value, leftAxisOut
                );
                rightDevice.method("TryGetFeatureValue", 2).invoke(
                    CommonUsages.field("primary2DAxis").value, rightAxisOut
                );
                ...
                if (Math.abs(lx) < 0.1 && Math.abs(ly) < 0.1 &&
                    Math.abs(rx) < 0.1 && Math.abs(ry) < 0.1) return;
                ...
                const flatForward = Vector3.method("Normalize").invoke(
                    Vector3.method("op_Subtraction").invoke(
                        headForward,
                        Vector3.method("op_Multiply").invoke(
                            up,
                            Vector3.method("Dot").invoke(headForward, up)
                        )
                    )
                );
                const flatRight = Vector3.method("Normalize").invoke(
                    Vector3.method("op_Subtraction").invoke(
                        headRight,
                        Vector3.method("op_Multiply").invoke(
                            up,
                            Vector3.method("Dot").invoke(headRight, up)
                        )
                    )
                );

                // right stick = forward/back and strafe
                const forwardVec = Vector3.method("op_Multiply").invoke(
                    flatForward, ry * speed * deltaTime
                );
                const rightVec = Vector3.method("op_Multiply").invoke(
                    flatRight, rx * speed * deltaTime
                );

                // left stick Y = up/down, left stick X = yaw (optional strafe)
                const upVec = Vector3.method("op_Multiply").invoke(
                    up, ly * speed * deltaTime
                );
            ...
            } catch (e) { console.error("joystick fly error:", e); }
        };
    })(),
    isTogglable: true,
    ...
    toolTip: "Stash spawner lol."
            ...
            new ButtonInfo({
                ...
                method: () => {
                    ...
                    if(rightGrab){console.log(rightHandTransform.method("get_position").invoke())}
                    //TryGrabObject
                    //Destroy(stick);
...
//
            ...
            new ButtonInfo({
              ...
              method: () => {
                ...
                if(rightSecondary && rightGrab){
                    PrefabGen.method("SpawnItem", 4).invoke(
                    Il2Cpp.string("mob_prefab/" + mobIDs[mobIndex]),
                    handTransform.method("get_position").invoke(),
                    handTransform.method("get_rotation").invoke(),
                    NULL
                    )
            ...
            new ButtonInfo({
              ...
              method: () => {
                ...
                for (let i = 0; i < mobIDs.length; i++) {   
                    PrefabGen.method("SpawnItem", 4).invoke(
                        Il2Cpp.string("mob_prefab/" + mobIDs[i]),
                        handTransform.method("get_position").invoke(),
                        handTransform.method("get_rotation").invoke(),
                        NULL
                    );
            ...
            //    }),
                ...
                method: () => {
                  ...
                  if (rightTrigger) {
                    try {
                      const hitPoint = ray.method("get_point").invoke();
                      const PrefabGen2 = AssemblyCSharp.class(
                        "AnimalCompany.PrefabGenerator"
                      );
                      const itemPrefab = PrefabGen2.method("GetItemPrefab").invoke(
                        Il2Cpp.string(itemIDs[itemIndex])
                      );
                      if (!itemPrefab || itemPrefab.handle.isNull()) {
                        sendNotification("Item prefab not found!", false);
                        console.error(
                          "Failed to get prefab for:",
                          itemIDs[itemIndex]
                        );
                        return;
                      }
                      ...
                      //if (!component || component.handle.isNull()) {
                       ...
                       // return;
                   //   }
                      PrefabGen2.method("SpawnItemAsync", 4).overload(
                        "AnimalCompany.GrabbableItemPrefab",
                        "UnityEngine.Vector3",
                        "UnityEngine.Quaternion",
                        // @ts-ignore
                        "Fusion.NetworkObjectSpawnDelegate"
                      ).invoke(itemPrefab, hitPoint, identityQuaternion, NULL);
                ...
                method: () => {
                  ...
                  if (rightTrigger) {
                    try {
                      ...
                      const itemPrefab = PrefabGen2.method("GetMobPrefab").invoke(
                        Il2Cpp.string(mobIDs[mobIndex])
                      );
                      if (!itemPrefab || itemPrefab.handle.isNull()) {
                        sendNotification("Item prefab not found!", false);
                        console.error(
                      ...
                      PrefabGen2.method("SpawnMobAsync", 4).overload(
                        "AnimalCompany.MobID",
                        "UnityEngine.Vector3",
                        "UnityEngine.Quaternion",
                        // @ts-ignore
                        "Fusion.NetworkObjectSpawnDelegate"
                      ).invoke(mobIDs[mobIndex], hitPoint, identityQuaternion, NULL);
                ...
                method: () => {
                    ...
                    if (fullbirhgtobject == null) {
                        fullbirhgtobject = createObject(
                            ...
                            getTransform(headCollider).method("get_rotation").invoke(),
                            [0, 0, 0],
                            3,
                            bgColor
                        );
...
//
        ...
        // Stash and Object stuff
    {
        ...
        else{ this.method("HandleKeyPressed").invoke(key); }
    };        
    }
